package com.github.bobocodeProjects.functionalJava04;
...
import com.github.bobocodeProjects.exception.ExerciseNotCompletedException;
...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
        		.collect(Collectors.partitioningBy((acc) -> acc.getSex().equals(Sex.MALE)));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
        		.collect(Collectors.groupingBy(acc -> acc.getEmail().split("@")[1]));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
        		.mapToInt(acc -> acc.getFirstName().length() + acc.getLastName().length())
        		.sum();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
        		.anyMatch(acc -> acc.getEmail().endsWith(emailDomain));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream()
        		.collect(Collectors.groupingBy((acc) -> acc.getBirthday().getMonth(),
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
        		.collect(Collectors.groupingBy((acc) -> acc.getCreationDate().getMonth(),
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames() {
    	return accounts.stream()
				...
				.flatMapToInt(String::chars)
				.mapToObj((c) -> String.valueOf((char) c).toLowerCase().charAt(0))
				.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }
